...
import com.prohect.mysql_frontend_server.sqlUtil.SqlUtil;
import com.prohect.mysql_frontend_server.sqlUtil.SqlUtil4Login;
...
public class Server {
    ...
    public void run() throws SQLException, IOException {
        ...
        try {
            ...
            new NettyServer(serverPort).run();
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private void processLoginPacket(ChannelHandlerContext ctx, CLoginPacket loginPacket) {
        try {
            ...
            if (user1 != null) {
                ...
                if (Objects.equals(user.getUuid(), user1.getUuid())) {
